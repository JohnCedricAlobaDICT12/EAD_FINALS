...
if (isset($_POST['login-btn'])) {
  ...
  $sql = "SELECT * FROM admin_tb WHERE Email = '$username' AND Pass = '$password'";
  ...
  if ($result && mysqli_num_rows($result) > 0) {
    ...
    Header ('Location:http://localhost/EcommerceAdminDashboad_main/Orderlists.php');
    exit();
  ...
  $username = $_POST['username'];
  $password = $_POST['password'];
  $sql = "SELECT * FROM managers_tb WHERE Email = '$username' AND Pass = '$password'";
  $result = mysqli_query($conn, $sql);
...
if (isset($_POST['reg-btn'])) {
    Header ('Location: http://localhost/EcommerceAdminDashboad_main/Loginadmin.php');
    exit();
    ...
    <form action="#" method="POST" class="asm-form active" id="frmSignIn" novalidate onsubmit="return validateForm(this)">
        ...
        <div class="form-group">
            <a href="Loginadmin.php">
                <button type="submit" class="btn btn-light btn-block" id="Reg-btn" name="reg-btn">Admin</button>
            </a>
        </div>
    </div>
    </form>
